...
void Player::update(sf::RenderWindow &gameWindow, float deltaTime)
{
    ...
    // Shoot bullet
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Space) && _canShoot) 
    {
    ...
    {
        _bullets[i].update(gameWindow, deltaTime);

        // if the bullet leaves the screen, delete it   
        if (_bullets[i].getPosition().y <= 0)
        {
